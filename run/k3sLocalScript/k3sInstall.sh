...
if [ "$1" = "local" ]; then
    if [ -s "/tmp/k3s.bin" ]; then
        LOCALK3SBIN=/tmp/k3s.bin
        echo "XXX using ${LOCALK3SBIN}"
    else
        echo "XXX local but /tmp/k3s.bin not found ... downloading"
    fi
fi
...
download_binary() {
    ...
    if [ -n "${LOCALK3SBIN}" ] && [ -s "${LOCALK3SBIN}" ]; then
            info "Copying ${LOCALK3SBIN} to \$TMP_BIN instead of downloading..."
            cp ${LOCALK3SBIN} ${TMP_BIN}
    else
